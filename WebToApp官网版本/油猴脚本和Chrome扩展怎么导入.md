...
比起临时写一段注入脚本，把现成的油猴脚本或 Chrome 扩展搬进来是更省事的做法 —— 你在浏览器上已经调好的脚本资产，有相当一部分可以直接复用。WebToApp 同时支持 **Tampermonkey / Greasemonkey 风格的 `.user.js`**（带 `GM_*` 接口）和 **Chrome MV3 格式的扩展**，导入后与其他扩展一样被解析、存储和注入。
